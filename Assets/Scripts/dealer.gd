...
extends Node2D
#TODO FIX LOBBY, FIX DEALER POS FOR OTHER AND IP SAVE SYSTEM CRYPTED
...
func show_result(result: String, index: int):
	var pos = game_manager.cards[game_manager.players[index].hand[0]].global_position - Vector2(0, 20)
	var element
	if result == "win":
		element = $Won
	elif result == "lose":
...
func hit():
	...
	await get_tree().create_timer(0.5).timeout
	var index = take_card()
	game_manager.cards[index].default_pos = table.get_tile_center(Vector2(1+player_turn_index* 2.0 + HELD_CARD_SEPARATION * game_manager.players[player_turn_index].hand.size(), 4))
